...
class HttpRequest(HttpMessage):
  ...
  def __init__(self, settings, env):
    ...
    # Do really basic initialization ...

    # self.settings = settings <----- this is done in parent constructor
    # self.env = env           <--'
    # self.files = self.get_vars = self.post_vars = self.cookies = None
    self.headers = CaseInsensitiveHttpEnv(self.env)
    self.method = self.verb = env.get('REQUEST_METHOD')  # You call it verb, I call it method
    ...
    self.is_chunked = 'chunk' in self.headers.get('TRANSFER_ENCODING', '').lower()
    # ^- well, it is so pro ;-)
